...
class ApiCalls {
  static Future<Object> loginCall(Logginer user) async {
    ...
    String request = Constants.getLoginUrl(
        user.username, user.password, "B7:EA:FA:A8:DB:54" /*user.macAddress*/);
    ...
    if (flag == 1) {
      ...
      return LoginError.fromJson(jsonDecsode);
  }
  ...
  static Future<CheckInResponse> checkService(CheckModel checkObject) async {
    ...
    mParams["Clients"] =
        checkObject.client == 'Client Name' ? " ' ' " : checkObject.client;
    mParams["AddressesInfo"] =
        checkObject.addressInfo != "" ? checkObject.addressInfo : " ' ' ";
...
/*    if(checkObject.fromWhere == from_itg) {
    ...
    mParams["FromWhere"] = checkObject.fromWhere == from_itg
        ? "1"
        : checkObject.fromWhere == from_home
            ? "2"
            : "3";
